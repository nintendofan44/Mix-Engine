...
class Alphabet extends FlxSpriteGroup {
	...
	public var playsoundontype:Bool = false;

	public var typingSpeed:Float = 0.05;

	public function new(x:Float, y:Float, text:String = "", ?bold:Bool = false, typed:Bool = false, ?typingSpeed:Float = 0.05, playsoundontype:Bool = true, ?textSize:Float = 1) {
		super(x, y);
		...
		this.text = text;
		this.typed = typed;
		this.playsoundontype = playsoundontype;
		isBold = bold;

		if (text != "") {
			if (typed) {
				startTypedText(typingSpeed, playsoundontype);
			}
			else {
				addText();
			}
	...
	public function startTypedText(speed:Float, playsound:Bool = true):Void {
		_finalText = text;
		doSplitWords();
